...
def get_timelen(config):
    ...
    test_all = config.timelens_test_root
...
def _load_events(is_timelens, file):
    ...
    print("keys in npz:", tmp.keys())
    if is_timelens:
        if "arr_0" in tmp:
            events = tmp["arr_0"].astype(np.float32)
...
def walk_video(is_timelens, video_path, key_frame, skip_frames, step=1):
    if is_timelens:
        image_template = join(video_path, "new", "*.png")
        info(f"image_template: {image_template}")
        event_template = join(video_path, "events_aligned", "*.npz")
        info(f"event_template: {event_template}")
        timestamps_path = join(video_path, "new", "info.txt")    #timestamp
    else:
    ...
    for i in range(0, len(images) - all_frames, step):
        ...
        sample.append(item)
        info(f"Constructing item{item} with {len(item['input_frames'])} input frames and {len(item['gt_frames'])} gt frames.")
    return sample
...
def walk(is_timelens, timelens_pp_root, key_frame, skip_frames, step=1):
    ...
    for video in videos:
        ...
        info(f"Constructing sample{video} with {len(sample)} samples.")
    return sample
...
class TimeLensPPDataset(Dataset):
    def __init__(
        ...
        if is_random_cropping:
            ...
            self.random_crop_resolution = self.low_resolution
